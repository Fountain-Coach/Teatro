...
Deliver an interactive preview window for FountainAI scripts and storyboards that displays live-rendered content, plays timeline MIDI events, and allows user interaction (playback, scrubbing, MIDI routing config) – all without regressing headless output determinism.
    •    id: teatro-gui-midi2
...
Provides a cross-platform windowing system and rendering loop for Teatro’s outputs, with live visualization of UMP streams (e.g., storyboard timelines, SSE token streams) and interactive controls to emit MIDI messages (playback, CC tweaks).
...
This should operate as a module that Teatro’s core can call into, preserving platform-neutral design. SDL integration should allow both onscreen and offscreen (headless) rendering using the same code paths where feasible.
### 2.    Interactive Rendering Loop
Establish a robust rendering loop that drives Teatro’s output to the screen in real time. Use SDL’s event polling and timing functions to tick at ~60 FPS. Synchronize with Teatro’s state updates, handle input events (close, pause/resume playback), and redraw the scene based on changes. Ensure clean exit and non-blocking operations.
### 3.    MIDI 2.0 Data Pipeline
Integrate the MIDI2 library to receive, decode, visualize, and emit UMP streams in real time. Support common message types (Note On/Off, CC, Program Change, Channel Voice, JR Timestamp). Render visual cues (piano roll, CC automation curves, activity indicators). Ensure non-blocking processing with timestamped synchronization.
### 4.    Interactive Controls & Playback
Provide GUI elements for playback control and scrubbing. Interactions map to UMP emissions. Example: play triggers Note On events; scrubbing adjusts timestamps. Map UI controls (sliders, knobs, keybindings) to CC messages. Ensure immediate visualization feedback.
